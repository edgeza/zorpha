...
/**
 * blockChunks, isLogLimitError and withAdaptiveRange used to live here, but
 * that made them import this file's module-scope `config` (via the import at
 * the top of this file) just to page through getLogs calls. season-snapshot.ts
 * needs exactly these three and nothing else from chain.ts, and must run
 * without RPC_URL/CHAIN_ID/SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY set, so the
 * implementations moved to the config-free ./log-paging.js and are re-exported
 * from here unchanged so index.ts's existing `from './chain.js'` import keeps
 * working.
 */
export { blockChunks, isLogLimitError, withAdaptiveRange } from './log-paging.js';
