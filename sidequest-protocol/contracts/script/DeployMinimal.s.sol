...
contract DeployMinimal is Script {
    /// @notice Env-driven entrypoint. Reads config, broadcasts, delegates.
    function run() external returns (VaultFactory factory, VaultLauncher launcher) {
        ...
        (factory, launcher) = deploy(deployer, gov, treasury, timelock, zor, targets);

        vm.stopBroadcast();

        _assertHandover(factory, launcher, deployer, gov, timelock);
        _report(factory, launcher, targets.length);
    }

    /// @notice The deployment itself, separated from env reading and
    ///         broadcasting so it can be executed somewhere other than a live
    ///         chain.
    ///
    /// @dev    Not cosmetic. A dry run against a mainnet FORK is the only way to
    ///         find out whether this script works before it is the transaction
    ///         that matters, and Foundry cannot run a broadcasting `run()` from
    ///         inside a test -- vm.startBroadcast makes the first renounceRole
    ///         fail with ReentrancySentryOOG. Splitting the logic out is what
    ///         makes test/fork/MinimalDeploy.t.sol able to exercise THIS code
    ...
    ) public returns (VaultFactory factory, VaultLauncher launcher) {
    ...
    function _report(VaultFactory factory, VaultLauncher launcher, uint256 venueCount) internal pure {
        console2.log("=== Zorpha minimal (launchpad, no oracle) ===");
        console2.log("VaultFactory     ", address(factory));
        console2.log("VaultLauncher    ", address(launcher));
        console2.log("Approved venues  ", venueCount);
        console2.log("");
